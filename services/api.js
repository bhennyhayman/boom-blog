...
export const fetchAllBlogs = async () => {
  const token = localStorage.getItem("token");
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/blog/allblogs`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
export const searchBlog = async(term) => {
   ...
   const token = localStorage.getItem("token");
    try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/blog/findblog?searchTerm=${term}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
